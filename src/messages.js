    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          row("訂單", orderNo(o.id)),
          row("上車", o.pickup),
          row("下車", o.destination),
          row("時間", o.ride_time || "尚未提供"),
          row("人數", o.passengers ? `${o.passengers} 位` : "尚未提供"),
          row("距離", `${Number(o.distance_km).toFixed(1)} 公里`),
          row("車程", `約 ${Math.ceil(Number(o.duration_min))} 分鐘`),
          { type: "separator" },
          row("起跳", `${o.base_fare} 元`),
          row("里程", `${o.mileage_fare} 元`),
          row("時間", `${o.time_fare} 元`),
          row("高速費", `${o.toll || 0} 元（預估）`),
          row("夜間加成", `${o.night_surcharge || 0} 元`),
          { type: "text", text: `預估車資：約 ${o.estimated_fare} 元`, weight: "bold", size: "xl", align: "center", margin: "lg" },
          ...(o.in_service_area ? [] : [{ type: "text", text: "此行程超出主要服務範圍，需由司機確認是否承接。", size: "xs", color: "#b00020", wrap: true }])
        ]
      },
      footer: {
